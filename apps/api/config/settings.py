...
class Settings(BaseSettings):
    ...
    # Database configuration — set DATABASE_URL to a Neon pooled connection
    # string in production (postgresql://...-pooler.../dbname?sslmode=require).
    ...
    # Shared secret between Auth.js (frontend, server-side only) and this
    # API's POST /api/auth/sync-user — never exposed to the browser.
    ...
    # Redis configuration — set REDIS_URL to Upstash's rediss:// connection
    # string in production; local dev falls back to host/port below.
    ...
    def _init_directories(self):
        """Initialize required local cache directories.

        On Vercel (and other serverless platforms) the filesystem is
        read-only outside of /tmp, and there's no point creating a local
        disk cache anyway — apps/ingest (run by GitHub Actions, which has
        a normal persistent-for-the-run filesystem) owns FastF1/API disk
        caching. This is a no-op there instead of raising on import.
        """
        import os
        ...
        for directory in directories:
            try:
                ...
                pass  # read-only filesystem or similar — not fatal
