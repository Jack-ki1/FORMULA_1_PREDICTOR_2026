...
export function FantasyPage(){
  ...
  useEffect(()=>{
    ...
    setPredError(null)
    // First try real FantasyScoring via backend fantasy/optimal (Monte Carlo + FantasyScoring)
    api.get<any>('/api/v1/fantasy/optimal?race_id=au&budget=100').then(res=>{
      if (res.ranked) {
        const fantasy: Record<string, number> = {}
        res.ranked.forEach((r:any)=> fantasy[r.driver_code] = Math.round(r.expected*10)/10)
        // Also fallback to predictions if ranked empty
        if (Object.keys(fantasy).length) {
          setPredicted(fantasy)
          setOptimal(res)
          return
        }
      }
      // Fallback to old JSX approximation if fantasy endpoint fails
      return api.post<any>('/api/v1/predictions', { race_id:'au', session_type:'race', simulation_count:1500 }).then(res2=>{
        const probs = res2.winner_probabilities || {}
        const fantasy2: Record<string, number> = {}
        Object.entries(probs).forEach(([code,p])=>{
          const prob = Number(p) || 0
          fantasy2[code] = Math.round((prob*25*1.4 + prob*8 + prob*6)*10)/10
        })
        setPredicted(fantasy2)
        setOptimal(res)
      })
    }).catch((e:any)=> {
      ...
      drivers.slice(0,10).forEach((d:any)=> {
        const hash = d.code.split('').reduce((a:number,c:string)=> a + c.charCodeAt(0),0) % 10
        mock[d.code]= Math.round((12 + hash)*10)/10
      })
      setPredicted(mock)
    })
  }, [drivers.length])
  ...
  return (
    <div className="px-4 sm:px-8 py-6 space-y-6">
      ...
      {/* Optimal Team from backend FantasyScoring + DNF-adjusted */}
      {optimal && (
        <div className="card p-4 border-2" style={{ borderColor:'#16a34a', background:'#f0fdf4'}}>
          <div className="f1-display font-bold flex items-center gap-2">Optimal Team — Solver <span className="fs-11 px-2 py-0.5 rounded-full bg-green-600 text-white">Monte Carlo + Knapsack</span></div>
          <p className="fs-11 text-sub mt-1">Backend <code className="f1-mono">FantasyScoring.get_optimal_picks</code> — greedy knapsack on predicted/price. Budget $100M.</p>
          <div className="grid sm:grid-cols-2 gap-2 mt-3">
            <div>
              <div className="fs-11 font-bold">Optimal 5 drivers</div>
              <div className="flex flex-wrap gap-1.5 mt-1">{Object.values(optimal.optimal?.optimal_picks || {}).map((code:any)=> <span key={code} className="px-2 py-1 rounded-full text-xs font-bold text-white" style={{ background: drivers.find((d:any)=> d.code===code)?.team_color||'#6b7280'}}>{code}</span>)}</div>
              <div className="fs-11 text-sub mt-1">Total expected: {optimal.optimal?.total_expected_points?.toFixed(1)} pts</div>
            </div>
            <div>
              <div className="fs-11 font-bold">Top ranked by expected</div>
              <div className="space-y-1 mt-1">{optimal.ranked?.slice(0,3).map((r:any)=> <div key={r.driver_code} className="flex justify-between fs-11"><span>{r.driver_code}</span><span>{r.expected.toFixed(1)} pts · {r.confidence.toFixed(2)} conf</span></div>)}</div>
            </div>
          </div>
          <button onClick={()=>{
            const picks = Object.values(optimal.optimal?.optimal_picks || {}) as string[]
            if (picks.length===5) { setSelectedDrivers(picks as string[]); setChip('extra_drs') }
          }} className="btn-primary mt-3 w-full" style={{ background:'#16a34a'}}>Apply optimal 5 to builder</button>
        </div>
      )}

      <div className="grid lg:grid-cols-2 gap-4">
        <div className="card p-4">
          <div className="f1-display font-bold">Value Picks — PPM Ranking <span className="fs-11 font-normal text-sub">(DNF-adjusted below)</span></div>
      ...
      <div className="card p-4">
        <div className="f1-display font-bold">DNF-Risk-Adjusted Value — points per million, DNF-adjusted</div>
        <p className="fs-11 text-sub">Cheap driver with 15% DNF risk needs to outscore reliable one by more to be worth it — sorted by (expected * (1-DNFrisk) / price). Serious players build spreadsheets for this; we generate it from reliability + Monte Carlo DNF.</p>
        <div className="mt-3 space-y-1">
          {(() => {
            const dnfAdj = drivers.map((d:any)=>{
              const pred = predicted?.[d.code]||0
              const price = driverPrices[d.code]||10
              const reliability = Number(d.reliability)||80
              const dnfRisk = (100 - reliability)/100
              const adj = pred * (1 - dnfRisk*0.5) // DNF penalty -20 is ~0.5x expected
              return { code:d.code, name:d.name, price, pred, dnfRisk, adj, ppmAdj: adj/price, color:d.team_color }
            }).sort((a,b)=> b.ppmAdj - a.ppmAdj).slice(0,6)
            return dnfAdj.map((v:any,i:number)=> (
              <div key={v.code} className="flex items-center gap-2 p-1.5 rounded-lg">
                <span className="f1-mono w-6 font-bold">{i+1}</span>
                <span className="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs" style={{ background: v.color}}>{v.code.slice(0,2)}</span>
                <span className="f1-mono w-10 font-bold">{v.code}</span>
                <span className="fs-11 w-12">{v.price}M</span>
                <span className="fs-11 w-14 text-amber-600">{(v.dnfRisk*100).toFixed(0)}% DNF</span>
                <div className="flex-1 h-2 bg-black/10 rounded-full overflow-hidden"><div className="h-full" style={{ width:`${Math.min(100, v.ppmAdj*30)}%`, background:'#f59e0b'}} /></div>
                <span className="fs-11 w-16 text-right font-bold">{v.ppmAdj.toFixed(2)} adj PPM</span>
                <span className="fs-11 w-12 text-right">{v.adj.toFixed(1)} adj pts</span>
              </div>
            ))
          })()}
        </div>
